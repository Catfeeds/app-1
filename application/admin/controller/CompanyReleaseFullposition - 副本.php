...
//        }
    ...
    public function selectWorkType()
    {
        if($this->request->param('type')!=''){
            ...
            //查找所有相匹配的类型
            $all_type = Db::name('WorkType')->where(array('name'=>array('like','%'.$type.'%')))->where('isdelete',0)->select();
            //若检索到了相对应的类型
            if($all_type){
                $type_name_array = array(); //用来存放所有的类型名称
                foreach($all_type as $val){
                    $type_name_array[] = $val['name'];
                }
    ...
    public function fsedit()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $typeInfo = Db::name('WorkType')->where('name',$type)->find();
...
//            }
            ...
            $salary1 = $salary_array[0]; //2k
            $salary2 = $salary_array[1]; //5k
    ...
    public function fadd()
    {
        ...
        if ($this->request->isPost()) {
            ...
            $salary1 = $this->request->param('salary1'); //最低薪资，如2k
            $salary2 = $this->request->param('salary2'); //最高薪资，如5k
            $data['salary'] = $salary1.'-'.$salary2; //薪资范围,用-分隔  如2k-5k
