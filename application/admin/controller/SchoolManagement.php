...
//          $map['release']=["like", "%" . $info1['realname']. "%"];
	...
	public function pay(){
			...
			$needpay = $this->request->param('needpay');
			$companyName = $this->request->param('companyName');
			$jianzhitype = $this->request->param('jianzhitype');
			$salaryUnit = $this->request->param('salaryUnit');
			$salaryMoney = $this->request->param('salaryMoney');
			$alaryH = $this->request->param('salaryH');
			$salaryMan = $this->request->param('salaryMan');
			$salaryDay = $this->request->param('salaryDay');
			$secure = $this->request->param('secure');

			$companyInfo = DB::name('SchoolManagement')->field('schoolAccount')->where('schoolName',$companyName)->find();
			$schoolAccount = $companyInfo['schoolAccount'];
		
			$this->view->assign('needpay',$needpay);
			$this->view->assign('companyName',$companyName);
			$this->view->assign('jianzhitype',$jianzhitype);
			$this->view->assign('salaryUnit',$salaryUnit);
			$this->view->assign('salaryMoney',$salaryMoney);
			$this->view->assign('alaryH',$alaryH);
			$this->view->assign('salaryMan',$salaryMan);
			$this->view->assign('salaryDay',$salaryDay);
			$this->view->assign('schoolAccount',$schoolAccount);
            $this->view->assign('secure',$secure);
	...
	{	
		$alipay = new Alipay();
		$total_amount = $this->request->param('needpay');//充值金额
		$companyName = $this->request->param('companyName');//公司
		$jianzhitype = $this->request->param('jianzhitype');//兼职类别
		$salaryUnit = $this->request->param('salaryUnit');//薪资单位
		$salaryMoney = $this->request->param('salaryMoney');//薪资
		$alaryH = $this->request->param('alaryH');//工作时间
		$salaryMan = $this->request->param('salaryMan');//招聘人数
		$salaryDay = $this->request->param('salaryDay');//用工天数
		$schoolAccount = $this->request->param('schoolAccount');
		$secure = $this->request->param('secure');
		
		$out_trade_no = date('YmdHis').rand(100000, 999999);//订单号

		$data['buynum'] = $total_amount;
		$data['schoolName'] = $companyName;
		$data['needposition'] = $jianzhitype;
		$data['unit'] = $salaryUnit;
		$data['salary'] = $salaryMoney;
		$data['worktime'] = $alaryH;
		$data['needpeople'] = $salaryMan;
		$data['workday'] = $salaryDay;
		$data['order_number'] = $out_trade_no;
		$data['schoolAccount'] = $schoolAccount;
		$data['secure'] = $secure;
		$data['buytime'] = date("Y-m-d H:i:s");


		$companyAccount = DB::name('AccountManagement')->insert($data);
